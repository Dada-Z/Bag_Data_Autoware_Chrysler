...
- `roscore` 
- `rosrun gps_extractor extract_speed.py`
...
Load `gps_data.csv` 
- attributes: timestamp, lat, lon, and hgt

Load `vehicle_imu_data.csv`
- attributes: timestamp, linear_accel_x, linear_accel_y, linear_accel_z, angular

Load `imu_data.csv` 
- attributes: timestamp, linear_accel_x, linear_accel_y, linear_accel_z, angular_vel_x, angular_vel_y, and angular_vel_z

Load `vehicle_speed.csv`
- attributes: timestamp, hor_speed_mps, ver_speed_mps, track_deg, speed_kmh

Merge by `timestamp` to obtain a single file that contains gps, imu, speed, and images information.
- create `match_features.py` file and load the following datasets
